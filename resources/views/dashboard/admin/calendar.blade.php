...
<div class="dashboard-header" style="display:flex; align-items:center; justify-content:space-between; gap:12px;">
    <div>
        ...
        <p class="text-sm text-gray-500">Kelola event dan jadwal tim Anda.</p>
    </div>

    <a href="{{ route('dashboard.admin.create') }}" style="background:var(--accent);color:white;padding:8px 12px;border-radius:6px;text-decoration:none;font-weight:700;">+ Add Event</a>
</div>
...
<div style="display:flex; gap:18px; margin-top:18px;">
                        ...
                        @if($cell)
                            ...
                            <div style="margin-top:6px; display:flex; flex-direction:column; gap:6px;">
                                @foreach($eventsByDay[$cell] ?? [] as $e)
                                    <a href="{{ route('dashboard.admin.edit', $e) }}" style="display:block; background:{{ $e->color ?? '#c7f9cc' }}; color:#04111b; padding:4px 6px; border-radius:4px; font-size:12px; text-decoration:none;">
    ...
    <div style="flex:1;">
        <div style="background:white;padding:12px;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,.06);">
            ...
            <div style="margin-top:12px; display:grid; grid-template-columns: repeat(7, 1fr); gap:8px; color:#6b7280;">
                @foreach(['Sun','Mon','Tue','Wed','Thu','Fri','Sat'] as $d)
            ...
            <div style="margin-top:14px;">
                {{-- a bigger calendar grid that shows events with colored pills --}}
                @php
                                        ...
                                        <div style="margin-top:8px; display:flex; flex-direction:column; gap:6px;">
                                            @foreach($eventsByDay[$day] ?? [] as $e)
                                                <div style="display:flex; justify-content:space-between; align-items:center; gap:8px;">
                                                    <a href="{{ route('dashboard.admin.edit', $e) }}" style="display:block; flex:1; padding:6px 8px; background:{{ $e->color ?? '#eef8ff' }}; border-radius:999px; color:#04111b; text-decoration:none; font-weight:600; font-size:13px;">
                                                        {{ \\Illuminate\\Support\\Str::limit($e->title, 50) }}
                                                    </a>
                                                    <form action="{{ route('dashboard.admin.destroy', $e) }}" method="POST" onsubmit="return confirm('Hapus event {{ addslashes($e->title) }}?');">
                                                        @csrf
                                                        @method('DELETE')
